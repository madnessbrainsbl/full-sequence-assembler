import sys,binascii,struct,time,os, stat, types, math
from os.path import join
from PowerBrick import *


print "===================================================================="
print "$Workfile:   cudacom.py  $",
print "$Revision: #1 $",
print "$Date: 2016/05/05 $"
print "===================================================================="
#      $Author: chiliang.woo $
...
#ujog_HC()
#ujog_HC1()
#GetTrack_wrt_ZnTbl_HC()
#restartDrv_brick():
#DisplayRWF_HC()
#DisplayRWF_HC2()
#__displayMemory_HC()
#PwrSwitch_brick(state='ON'):
#b=enumerate()
#SetMuxBon() for Ctrl G (special code)
#rrc_bank_HC()
#readChnlSymb_HC()
#writeChnlSymb_HC
#ser_hc()
...
if not 'processRequest70' in locals():
   def processRequest70(requestData, *args, **kargs):
     ...
     if requestKey == 70:
       ...
       strr = "Unsupported requestKey in processRequest70==>",requestKey
     ...
     if requestBlock < runtBlock:
       ...
       strr = "processRequest70(): Request for block %s is beyond than last block %s." % (requestBlock,runtBlock)
       print str
       raise FOFParameterError,strr
...
def __displayMemory(buf, baseAddr=0, addressableUnit=1, unitsPerLine=16, groupSize=4, startingByte=0, forceLen=0):
  ...
  ####################################################################
def __displayMemory_HC(buf, baseAddr=0, addressableUnit=1, unitsPerLine=16, groupSize=4, startingByte=0, forceLen=0):
  """
  Print a binary string buffer to the string in a "memory dump" format::
      address     hex data for 0..(N-1)
      address+N   hex data for N..(2N-1)
  ...
  cnt=0
  print "Address   Data\n",
  ScriptComment("Address   Data")
  # Convert addressableUnit from bytes to nibbles
  addressableUnit*=2
  strr=''
  print '%08x ' % baseAddr,
  strr+=str('%08x ' % baseAddr)
  
  for i2 in range(startingByte,startingByte+bytesToDisplay,addressableUnit):
    print data[i2:i2+addressableUnit],
    strr+=data[i2:i2+addressableUnit]
    strr+=' '
    cnt+=1
    if cnt % groupSize == 0:
      print " ",
  ...
  ScriptComment('')
####################################################################
def DisplayRWF_HC(Hd=0,StartZone=0,EndZone=24,Channel='Yarra'):
  """
  Display ReadChannel related I table values in SF3
  @param Hd:                Head of concern
  @param StartZone:         Start Zone to display 
  @param EndZone:           End Zone to display
  @param Channel:           Channel of the drive. Current support Yarra (B9100 and LuxorM)
  """
  
  #check max head of the drive
  for hh in [3,2,1]:
    buf, errorCode = fn(1361, hh)
    firstbyte = struct.unpack('BB',buf[0:2])
    if firstbyte[0]<127:   #has such head
        total_hd=hh+1
        break
  
  #get current track
  buf, errorCode = fn(1362)
  CurrentTrack = struct.unpack(">LB",buf)
  #seek to other hd
  mCyl, lCyl = lwords(CurrentTrack[0])
  fn(1243, lCyl, mCyl, (CurrentTrack[1]+1)%total_hd, 0x15)
  
  #tuple structure: for different bank. Bank number is the first entry. 
  #for this case we are concern with Bank 0, 1, 9 for Yarra,  Bank 0, 2, for LuxorM
  if Channel=='Yarra':
    RWFTable_List_Bank= ((0,0x1A, 0x1C, 0x20 ,0x22,0x24 ,0x26,0x28,0x2A,0x2C,0x32,0x34,0x36,0x38,0x3a ),
    (1,0x04, 0x06, 0x08 ,0x12),(9, 0x20, 0x22))
  else:
    if Channel=='LuxorM':
        RWFTable_List_Bank= ((0,0x1A, 0x1C, 0x20 ,0x22,0x24 ,0x26,0x28,0x2A,0x2C,0x32,0x34,0x36,0x38,0x3a,
        0x44,0x46,0x48,0x52 ), (2,0x60, 0x62))
            
  print ' '
  ScriptComment(' ')
  print 'Head %d' %Hd
  ScriptComment('Head %d' %Hd)
  RWF='Zone  '
  for j in range(len(RWFTable_List_Bank)):
      for i in RWFTable_List_Bank[j][1:len(RWFTable_List_Bank[j])]:
        RWF+="0x%02X " %(i+RWFTable_List_Bank[j][0]*0x100)
  print RWF
  ScriptComment(RWF)
  Tbar='='
  for i in range(len(RWF)-2):
    Tbar+='='
  print Tbar
  ScriptComment(Tbar)
     
  for i in range (StartZone,EndZone+1):
    RWF=str('%04X  ' % i)
    mCyl, lCyl = lwords(GetTrack_wrt_ZnTbl_HC(Hd,i,TrkOffset=100,echo='off'))
    fn(1243, lCyl, mCyl, Hd, 0x15)
     
    for bb in range(len(RWFTable_List_Bank)):
        if RWFTable_List_Bank[bb][0] == 0:
            fn(1269, 0x2a, 0x8018,-1,-1,0xffff,Echo=0)  # to display FIR values , set Reg 0x2a to 0x8018
        buf,errorCode = fn(1324,RWFTable_List_Bank[bb][0])
        data = binascii.hexlify(buf)
        if RWFTable_List_Bank[bb][0] == 0:
            fn(1269, 0x2a, 0xf018,-1,-1,0xffff,Echo=0)  # set back Reg 0x2a to 0xf018
        for ii in RWFTable_List_Bank[bb][1:len(RWFTable_List_Bank[bb])]:
            RWF+=data[ii*4:ii*4+4]
            RWF+=' '
            if RWFTable_List_Bank[bb][0]>0:
                RWF+=' '
    print RWF
  ...
  mCyl, lCyl = lwords(CurrentTrack[0])     #seek back to the current track
  fn(1243, lCyl, mCyl, CurrentTrack[1], 0x15)
  
    
####################################################################

def DisplayRWF2_HC(Hd=0,StartZone=0,EndZone=24,Channel='Yarra'):
  """
  Display ReadChannel related I table values in SF3
  @param Hd:                Head of concern
  @param StartZone:         Start Zone to display 
  ...
  #for this case we are concern with Bank 0, 1, 9 for Yarra,  Bank 0, 2, for LuxorM
  if Channel=='Yarra':
    RWFTable_List_Bank= [0x1A, 0x1C, 0x20 ,0x22,0x24 ,0x26,0x28,0x2A,0x2C,0x32,0x34,0x36,0x38,0x3a ,0x104, 0x106, 
    0x108 ,0x112 ,0x920, 0x922]
  else:
    if Channel=='LuxorM':
        RWFTable_List_Bank= [0x1A, 0x1C, 0x20 ,0x22,0x24 ,0x26,0x28,0x2A,0x2C,0x32,0x34,0x36,0x38,0x3a,
        0x44,0x46,0x48,0x52,0x260, 0x262]
  ...
  for i in range (StartZone,EndZone+1):
    RWF=str('%04X  ' % i)
    mCyl, lCyl = lwords(GetTrack_wrt_ZnTbl_HC(Hd,i,TrkOffset=100,echo='off'))
    fn(1243, lCyl, mCyl, Hd, 0x15)   #read seek
     
    for bb in range(len(RWFTable_List_Bank)):
        if RWFTable_List_Bank[bb]>>4 == 2:
            fn(1269, 0x2a, 0x8018,-1,-1,0xffff,Echo=0)  # to display FIR values , set Reg 0x2a to 0x8018
        buf, errorCode = fn(1308, RWFTable_List_Bank[bb])   #rrc command
        RWF+= binascii.hexlify(buf)
        RWF+=' '
...
def reg(address, size = 16, writeFlag = 0, data = 0):
  ...
  mswData = 0
  if size == 32:
      mswData, lswData = lwords(data)
  mswAddress, lswAddress = lwords(address)
...
def dset(section=0, offset=0, pattern=0xC6, count=32):
  ...
  mOffset, lOffset = lwords(offset)
  mBytes,  lBytes  = lwords(count)
...
def esk( cyl, head, seektype, displayOptions ):
  ...
  mCylinder, lCylinder = lwords(cyl)
...
def Write_STCircularBuf(NO_SEEK=0, cyl=0xFFFFFFFF, head=0xFFFF, delay=0, offset=0):
  """
  ...
  mCylinder, lCylinder = lwords(cyl)
  buf, errorCode = fn(1378, lCylinder, mCylinder, head, delay, offset, NO_SEEK)
  #buf,errorCode = __ReceiveResults()
  if NO_SEEK != 0:
	  __displayBuffer(buf)
  else:
...
def ssv(firstWedge=0, wedgeCount=-1,
  ...
  return buf
    ...
    ####################################################################
def restartDrv_brick():
    #restart
    print "Restart Drive"
    PowerBrick.cycle(b[0],2)
    time.sleep(2)
    SetBaudRate(38400)
    SetESlipMidBaud()
    #st(1)
 ...
 ####################################################################
def PwrSwitch_brick(state='ON'):
    #restart
    if state=='ON':
        PowerBrick.switch(b[0],PowerBrick.ON)
        time.sleep(2)
        SetBaudRate(38400)
        SetESlipMidBaud()
    else:
        PowerBrick.switch(b[0],PowerBrick.OFF)
        time.sleep(2)
...
def FastIOSetup(NumServoValues = 2, NumChannelValues = 0, SymChanMask = 0, SymChan0 = 30, SymChan1 = 3, SymChan2 = 0, SymChan3 = 0, SymChan4 = 0, SymChan5 = 0):
  """
  Sets up the Servo Symbol Table Indexes or Channel Addresses to collect for the Fast IO function below.
  @param NumServoValues: The total number of Servo Symbol Table Index entries 
  @param NumChannelValues: The total number of Channel Address Entries
  @param SymChanMask: Bit mask where 1 indicates Channel Address and 0 indicates Servo Symbol Table Index and bit 0 is for SymChan0 and
                      bit 1 is for SymChan1 etc. up to the sum of NumServoValues and NumChannelValues.
  @param SymChan0: First Servo Symbol Table Index or Channel Address
  @param SymChan1: Second Servo Symbol Table Index or Channel Address
  @param SymChan2: Third Servo Symbol Table Index or Channel Address
  @param SymChan3: Fourth Servo Symbol Table Index or Channel Address
  @param SymChan4: Fifth Servo Symbol Table Index or Channel Address
  @param SymChan5: Sixth Servo Symbol Table Index or Channel Address
  @return: Error code
  """
  global gFastIOSetup
  gFastIOSetup = 1
  ValMask = (NumServoValues << 12) + (NumChannelValues << 8) + SymChanMask
  buf, errorCode = fn(1208, SymChan0, SymChan1, SymChan2, SymChan3, SymChan4, SymChan5, ValMask)
#  __displayMemory(buf,0)
  return errorCode

####################################################################

def FastIO(WriteSkipLength_A, WriteTransferLength_B, ReadSkipLength_C, ReadTransferLength_D, TrailingTransferLength_E, Pattern=0xc0c0, WriteXferLen = 100, ReadXferLen = 50, Flags = 0x0007, Flags2 = 0x0046, Revs = 2):
  """
  Calls the Fast IO function
  NOTE: Must call above function FastIOSetup to setup the Servo Symbol Table Indexes or Channel Addresses first.
  @param WriteSkipLength_A: FAST IO "A" WriteSkipLength
  @param WriteTransferLength_B: FAST IO "B" WriteTransferLength
  @param ReadSkipLength_C: FAST IO "C" ReadSkipLength
  @param ReadTransferLength_D: FAST IO "D" ReadTransferLength
  @param TrailingTransferLength_E: FAST IO "E" TrailingTransferLength
  @param Pattern: One word write pattern
  @param WriteXferLen: Percent of each wedge to write
  @param ReadXferLen: Percent of each wedge to read
  @param Flags: Fast IO Flags, see definitions below
  @param Flags2: More Fast IO flags, see definitions below
  @param Revs: Number of revs to write or read
  @return: Error code

  Flags Definition:
  =================
   // These defines are used by flags member of structure fast_io_data_collection_dfb_data_rev1
   #define  DIAG_FAST_IO_LIMIT_NUM_XFR_WEDGES                        BIT0
   #define  DIAG_FAST_IO_LIMIT_WEDGE_XFR_SIZE                        BIT1
   #define  DIAG_FAST_IO_DISABLE_SEEK                                BIT2
   #define  DIAG_FAST_IO_UNUSED_BIT3                                 BIT3
   #define  DIAG_FAST_IO_RANDOMIZED_DATA_WEDGE_WRITE                 BIT4

   // These defines are used by flags2 member of structure fast_io_data_collection_dfb_data_rev1 via the fast_io_control_block structure
   #define  DIAG_FAST_IO_READ                                        BIT0
   #define  DIAG_FAST_IO_WRITE                                       BIT1
   #define  DIAG_FAST_IO_COLLECT_SERVO_DATA                          BIT2
   #define  DIAG_FAST_IO_READ_CHANNEL_DATA                           BIT3
   #define  DIAG_FAST_IO_HEAT_ONLY                                   BIT4
   #define  DIAG_FAST_IO_START_XFR_ON_FIRST_AVAILABLE_WEDGE          BIT5
   #define  DIAG_FAST_IO_READ_WITH_HEAT                  	         BIT6
   #define  DIAG_FAST_IO_CHANNEL_DATA_VALID_UNTIL_RG_FALLING_EDGE    BIT7
   #define  DIAG_FAST_IO_USE_SM_WITH_UNFORMATTED_XFR                 BIT8
   #define  DIAG_FAST_IO_BYPASS_WRITE_PATTERN_SETUP                  BIT9  // Bypass the write pattern setup for FastIO collection.
   #define  DIAG_FAST_IO_TWO_LEVEL_WRITE_HEAT                        BIT10
   #define  DIAG_FAST_IO_TWO_DELAY_HEAT_VALUES                       BIT11
   #define  DIAG_FAST_IO_READER_HTR_ACTIVE                           BIT12        
  """
  global gFastIOSetup
  if gFastIOSetup == 0:
      print "FastIOSetup not Called!!! Call FastIOSetup before executing FastIO."
      return 10264
  print "This command is still under construction. Only writes and servo symbol table data colleciton have been verified."
  print "Initial working version check-in. For any questions, please contact the author of this command."
  print "This warning message will be removed when the final revision is released."
  WriteSkipLength_A = WriteSkipLength_A + (Flags * 256)                            # Hi: Flags, Low: WriteSkipLength_A
  TrailingTransferLength_E = TrailingTransferLength_E + (ReadSkipLength_C * 256)   # Hi/Lo
  Revs = Revs + (WriteXferLen/2 * 1024) + (ReadXferLen/2 * 16)                   # WWWW WWRR RRRR REVS W=WriteXferLen R=ReadXferLen REVS=Flags
  buf, errorCode = fn(1207, WriteSkipLength_A, WriteTransferLength_B, ReadTransferLength_D, TrailingTransferLength_E, Revs, Flags2, Pattern)
  #__displayMemory(buf,0)
  return errorCode
...
def rrc(Address, Echo = 1):
  ...
  return errorCode
#####################################################################
def rrc_bank_HC(Address, NumReg=10,DispEvenRegOnly=1,Echo = 1):
  """
  Read and display Channel register
  @param Address: Channel register to read
  @param Echo:     Display buffer data.
  ...
  for i in range(int(Address),int(Address)+NumReg,DispEvenRegOnly+1):
    strr+='%04X ' %(i)
  print strr
  strLen=len(strr)
  strr='-'
  ...
  strr='%08x ' % Address
  for i in range(int(Address),int(Address)+NumReg,DispEvenRegOnly+1):
    buf, errorCode = fn(1308, i)
    strr+= binascii.hexlify(buf)
    strr+=' '
    #buf,errorCode = __ReceiveResults()
    #if ( Echo == 1 ):
    #    __displayMemory(buf,Address,addressableUnit=2)
...
def ser(TLevel = 0):
  ...
  @param Tlevel: TLevel for PRML channels or iteration margin for LDPC channels
  @return: Error code
  """
  buf, errorCode = fn(1341, TLevel)
  #buf,errorCode = __ReceiveResults()
  result = struct.unpack("fHHHHLLLLL",buf)
  print "SER = %f " % (result[0:1]) + "Data Errors = %u " % (result[1:2]) + "Sync Errors = %u " % (result[2:3]) + \
  "Total Errors = %u " % (result[3:4]) + "#Sectors = %u " % (result[4:5]) + "Sense Data = %x" % (result[5:6])
  return errorCode
####################################################################

def ser_hc(TLevel = 0,Num_rev=20,printStr=1):
  """
  Run the VBAR sector error rate command using ActiveHead and ActiveCylinder for the target location
  Display Sector Error Rate, Data Errors, Sync Errors, Total Errors, #Sectors Read, and Sense Code
  @param Tlevel: TLevel for PRML channels or iteration margin for LDPC channels
  @Tlevel 0xAB (A outer iteration, B inner iteration for M93)
  @return: Error code
  """
  Data_Errors=0.0
  Sectors=0.0
  for i in range(Num_rev):
      ...
      #buf,errorCode = __ReceiveResults()
      result = struct.unpack("fHHHHLLLLL",buf)
      Data_Errors += result[1]
      Sectors += result[4]
  if Data_Errors==0:
    strr= "SER = %f " % (log10(Sectors*4096*8.0)) + "Data Errors = %u " % (Data_Errors) + "#Sectors = %u " % (Sectors) 
  else:
    strr= "SER = %f " % (-log10(Data_Errors*1.0)+log10(Sectors*4096*8.0)) + "Data Errors = %u " % (Data_Errors) + "#Sectors = %u " % (Sectors) 
  if printStr==1:
    print strr
  return strr
####################################################################
...
def ber(targetBer, numRevs, TLevel, ldpc=0, globalIterations = 0, localIterations = 0, zeroLatency=0, timeout=30):
  """
  ...
  @param timeout: Test time required for BER test, default of 30 covers up to 1000 revs.
  @return: Error code
  """
  if ((TLevel % 2) != 0) or ((TLevel > 30) and (TLevel != 100)):
    ...
    return
  if ldpc & 1:
      zeroLatency |= 4   # Use this flag to set LDPC SFR option for execfunc.c
  if ldpc & 2:
      zeroLatency |= 2   # Use this flag to set LDPC BER option for execfunc.c
  print targetBer, numRevs, TLevel, globalIterations, localIterations,  zeroLatency
  ...
  if ldpc != 0:
      ...
      other_errors = result[4:5]
      print "BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[2:3]) + "Sync Errors = %u " % (result[3:4]) + \
      "Other Errors = %u " % (result[4:5]) + "Sectors per Rev = %u " % (result[5:6]) + "Sense Data = %x " % (result[7:8])
  if other_errors[0] > 0:
      if ldpc != 0:
...
def ReadRetryBuckets(head = 0xFFFF, cylinder = 0xFFFFFFFF, Tlevel = 10, timeout = 30):
  """
  Run ReadRetryBuckets. Display error rate, sectors in error, and number of retries
  Indicate hard or soft error
  @param head: Head select. Default is ActiveHead
  @param cylinder: Track to test. Defualt is ActiveCylinder.
  ...
  i = 22
  mCylinder, lCylinder = lwords(cylinder)
  buf, errorCode = fn(1374, head, lCylinder, mCylinder, Tlevel)
  #buf,errorCode = __ReceiveResults(timeout)
  result = struct.unpack("LLLLLH",buf[0:22])
  print "Number of sectors with no retry or correction: %d" % result[0]
  print "Number of sectors with no retry, but correction needed: %d" % result[1]
  print "Number of sectors with soft error, or below threshold retry count: %d" % result[2]
  print "Number of sectors with hard error, or above threshold retry count: %d" % result[3]
  print "Number of unreadable sectors: %d" % result[4]
  print "Sectors per Rev is: %d" % result[5]
  print ""
  j = 0
  #222 = 22 bytes of count data + 50 * 4 bytes of sector-specific data
  while i<222:
...
def ujog_HC(start_offset, end_offset, step_size, mode = "BER",targetBer=90, numRevs=0, TLevel=0, ldpc=1,globalIterations=1, localIterations=20,  zeroLatency=0):
  """
  Microjog: Measure BER or CQM versus offtrack percent on the ActiveHead, ActiveCylinder.
  Display offtrack, BER or CQM, Data Errors, Sync Errors, Other Errors, and Sense Code.
  @param start_offset: starting offtrack - OD side
  @param end_offset: ending offtrack - ID side
  @param step_size: offtrack step size
  @param mode: BER (default) or CQM to be used as the metric or SER (call ser())
  @return: Error code
  """
  errorCode = 0
  if (start_offset < -128) or (end_offset > 128):
  ...
  while end_offset >= offset and errorCode == 0:
    ...
    offtrack = offset / 2.56
    if mode == "BER":
#      ber(targetBer,numRevs,TLevel,ldpc,globalIterations,localIterations,zeroLatency)
      if ldpc & 2:
        zeroLatency |= 2   # Use this flag to set the BER option for execfunc.c
      buf, errorCode = fn(1342, targetBer, numRevs, TLevel, globalIterations, localIterations,  zeroLatency)    # BER test
      if ldpc != 0:
        result = struct.unpack("fLLLLHHLL",buf)
      else:
        result = struct.unpack("fLLLHHLLL",buf)
      if ldpc == 1:
        other_errors = result[4:5]
        print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[2:3]) + "Sync Errors = %u " % (result[3:4]) 
        ScriptComment( "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[2:3]) + "Sync Errors = %u " % (result[3:4]) )
      elif ldpc == 2:
        other_errors = result[4:5]
        print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Bits in Error = %u " % (result[1:2]) + "Sync Errors = %u " % (result[3:4]) 
        ScriptComment("Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Bits in Error = %u " % (result[1:2]) + "Sync Errors = %u " % (result[3:4]) )
      else:
        other_errors = result[3:4]
        print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[1:2]) + "Sync Errors = %u " % (result[2:3]) 
        ScriptComment( "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[1:2]) + "Sync Errors = %u " % (result[2:3]) )
      
      #result = struct.unpack("fLLLLHHLL",buf)
      #print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER: %5.2f" % (result[0:1]) + " Data Errors: %u " % (result[2:3])
    #  if other_errors[0] > 0:
    #      print "Failed Sector = %u" % (result[6:7]) + "Sense Data = %x" % (result[5:6])
    elif mode == "SER": # call ser()
        strr=ser_hc(TLevel,numRevs,0)
        print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f " % offtrack + strr
        ScriptComment("Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f " % offtrack + strr)
    else:   #call 'CQM' mode
      cqmResult = cqmAgere(50,1) # Measure CQM over 50 sectors in quiet mode
      print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " CQM: %7u" % cqmResult
    offset = offset + step_size
...
def ujog_HC1(DType, Hd, Zn,start_offset, end_offset, step_size, mode = "BER",targetBer=90, numRevs=0, TLevel=0, ldpc=1,globalIterations=1, localIterations=20,  zeroLatency=0):
  """
  Microjog: Measure BER or CQM versus offtrack percent on the ActiveHead, ActiveCylinder.
  Display offtrack, BER or CQM, Data Errors, Sync Errors, Other Errors, and Sense Code.
  @param start_offset: starting offtrack - OD side
  ...
  offset = start_offset
  if ldpc == 1:
    print "Drive Hd Zn ECC OfftrackPct BER"
    ScriptComment( "Drive Hd Zn ECC OfftrackPct BER")
  while end_offset >= offset and errorCode == 0:
    ofs(offset)
    offtrack = offset / 2.56
    if mode == "BER":
#      ber(targetBer,numRevs,TLevel,ldpc,globalIterations,localIterations,zeroLatency)
      ...
      if ldpc == 1:
        other_errors = result[4:5]
        #print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[2:3]) + "Sync Errors = %u " % (result[3:4])
        print "%3s" % DType +"%5u" % Hd +"%5u" % Zn +"%5u" % TLevel + " %7.3f" % offtrack + " %f " % (result[0:1]) 
        ScriptComment("%3s" % DType +"%5u" % Hd +"%5u" % Zn +"%5u" % TLevel + " %7.3f" % offtrack + " %f " % (result[0:1]) )
      elif ldpc == 2:
        other_errors = result[4:5]
        print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Bits in Error = %u " % (result[1:2]) + "Sync Errors = %u " % (result[3:4]) 
        ScriptComment("Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " BER = %f " % (result[0:1]) + "Bits in Error = %u " % (result[1:2]) + "Sync Errors = %u " % (result[3:4]) )
...
def sweep(reg, start_val, end_val, step_size, mode = 0):
  ...
  while parmVal < end_val and errorCode == 0:
      ...
      if testMode == "BER":
          ...
          result = struct.unpack("fHHHHLLLLL",buf)
          other_errors = result[3:4]
          print "Parm Value: %3u" % parmVal + " BER: %5.2f" % (result[0:1]) + " Data Errors: %u " % (result[1:2]) + "Sync Errors: %u" % (result[2:3])
          if other_errors[0] > 0:
              print "Failed Sector = %u" % (result[6:7]) + "Sense Data = %x" % (result[5:6])
      else: # mode must be CQM
          cqmResult = cqmAgere(50,1) # Measure CQM over 50 sectors in quiet mode
...
def sweep_HC(reg, start_val, end_val, step_size, mode = 0,targetBer=90, numRevs=0, TLevel=0, ldpc=1,globalIterations=1, localIterations=20,  zeroLatency=0):
  """
  Run a sweep on input parm vs BER or CQM at the cudacom global head and cylinder values.
  Display Parm value tested, BER or CQM, Data Errors, Sync Errors, Other Errors, and Sense Code.
  @param start_val: starting parameter value
  ...
  buf, errorCode = fn(1338, reg)
  result = struct.unpack("HH",buf)
  OriginalRegValue = (result[0:1])
  print "OriginalReg: 0x%x" % (reg) +"  Value: 0x%x " % (OriginalRegValue)
  ...
  if (mode & 2) == 2:   # 0000 no writes,  0002 writes required
      numWrites = 1
  parmVal = start_val
  for parmVal in range (start_val, end_val, step_size):
      if  errorCode > 0:
          break
      wrcf(reg, parmVal, gZone, gHead)
      if numWrites > 0:
      ...
      if testMode == "BER":
        ...
        if ldpc == 1:
            other_errors = result[4:5]
            print "Parm Value: 0x%2X" % parmVal +  " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[2:3]) + "Sync Errors = %u " % (result[3:4]) 
            ScriptComment( "Parm Value: 0x%3X" % parmVal +  " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[2:3]) + "Sync Errors = %u " % (result[3:4]) )
        elif ldpc == 2:
            other_errors = result[4:5]
            print "Parm Value: 0x%2X" % parmVal +  " BER = %f " % (result[0:1]) + "Bits in Error = %u " % (result[1:2]) + "Sync Errors = %u " % (result[3:4]) 
            ScriptComment("Parm Value: 0x%3X" % parmVal +  " BER = %f " % (result[0:1]) + "Bits in Error = %u " % (result[1:2]) + "Sync Errors = %u " % (result[3:4]) )
        else:
            other_errors = result[3:4]
            print "Parm Value: 0x%2X" % parmVal +  " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[1:2]) + "Sync Errors = %u " % (result[2:3]) 
            ScriptComment( "Parm Value: 0x%3X" % parmVal +  " BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[1:2]) + "Sync Errors = %u " % (result[2:3]) )
  
      else: # mode must be CQM
          cqmResult = cqmAgere(500,1) # Measure CQM over 50 sectors in quiet mode
          print "Parm Value: 0x%2X" % parmVal + " CQM: %7u" % cqmResult
      
  wrcf(reg,int(OriginalRegValue[0]),gZone, gHead)
  return errorCode
####################################################################
...
def rpre(Address,Page=0):
  ...
  @return: Error code
  """
  buf, errorCode = fn(1329, Address, Page)
  #buf,errorCode = __ReceiveResults()
  bufBytes = struct.unpack('BB',buf)
  print "%02x:" % Address,
  print " %02x" % bufBytes[0]    # register value
  return errorCode
####################################################################

def wpre(Address, Data, Page=0):
  """
  Write Preamp register
  @param Address: Preamp register to write
  @param Data:    Value to write
  @param Page: Page select. Default zero, non zero value selects second page.
  @return: Error code
  """
  buf, errorCode = fn(1330, Address, Data, Page)  # write
  #__ReceiveResults()
  return rpre(Address, Page)      # read back
####################################################################

def dumpPreamp(StartAddress=0,EndAddress=15,Page=0):
  """
  Dump Preamp registers
  @param StartAddress: Starting preamp register
  @param EndAddress: Ending preamp register
  @param Page: Page select. Default zero, non zero value selects second page.
  @return: Error code
  """
  buf, errorCode = fn(1332, StartAddress, EndAddress, Page)
  #buf,errorCode = __ReceiveResults()
  __displayMemory(buf,StartAddress)
  return errorCode
...
def reloadRap(type=0, option=0):
  ...
  @return: Error code
  """
  buf, errorCode = fn(1333, type, option)
  #buf,errorCode = __ReceiveResults()
  return errorCode
####################################################################
...
def dumpBank(bank=0):
  """
  Bank 0 for Channel Adaptive parameters
  Bank 1 for TBG related format parameters
  Bank 9 for Asymmetry
...
def dumpBank_HC(bank=0):
  ...
  #buf,errorCode = __ReceiveResults()
  print "Bank %d" % bank
  ScriptComment("Bank %d" % bank)
  __displayMemory_HC(buf, 0, addressableUnit=2)
  return errorCode
...
'''
####################################################################
def dumpChannel(startAddr = 0, endAddr = 0x3E6):
  ...
  addr = 0
  datas = []
  while startAddr <= addr <= endAddr and errorCode == 0:
    buf, errorCode = fn(1308, addr)
    datas.append(binascii.hexlify(buf))
    addr += 1
  if errorCode == 0:
    for i in range(dispCOL):
        print "col%.2d " % i,
    print '\n'
    for index in range(len(datas)):
      if (index + 1) % dispCOL != 0:
        print " %s " % datas[index],
      else:
        print " %s " % datas[index]
  else:
    print " error to read channel registers"
  return errorCode
####################################################################
...
def DLCurrResultsFile():
  ...
  try:
     ...
     strr = "Cannot download results file!!"
     raise ScriptTestFailure,strr
###########################################################################
...
def processRequest16(requestData, *args, **kargs):
  ...
  if requestKey == 16:
    ...
    strr = "Unsupported requestKey in processRequest16==>",requestKey
    raise FOFParameterError,strr
  ...
  if requestBlock < runtBlock:
    ...
    strr = "processRequest16(): Request for block %s is beyond than last block %s." % (requestBlock,runtBlock)
    raise FOFParameterError,strr
...
def logver( cyl, code_hd, pos, len, pattern):
  ...
  """
  mCylinder, lCylinder = lwords(cyl)
  mPosition, lPosition = lwords(pos)
...
def GetTrack_wrt_ZnTbl_HC(head=0,Zzn=14,TrkOffset=100,echo='on'):
  """
  Display Zone Table for head specified by the input param, including 4 items at present:
  Head     Zone#       StartCyl        EndCyl
  Item "Head" will display only when VBAR supported.
  TrkOffset Specifies the target cylinder within the each zone. 
  The valid range is 0 to 199 decimal. Each 'tick' is one half of one percent - i.e., set this to 100 decimal
  to specify testing in the middle of the zone (50%); set it to 199 decimal to specify testing in the inner radius of the zone (99.5%).
  @param head: limited by buffer size, it displays zone table for only one head each time when VBAR supported.
  @return: Error code   
  """
  buf, errorCode = fn(1361, head)
  ...
  #for i in range(0, len(buf), 10):
  line1 = struct.unpack('BB', buf[Zzn*10 : Zzn*10+2])    #'BBLL' does not work, so split it into two lines
  hd, zn = line1
  line2 = struct.unpack('LL', buf[Zzn*10+2 : Zzn*10+10])
  starCyl, endCyl = line2
  #ReturnTrack=int(((endCyl-starCyl)/200*TrkOffset)+starCyl)
  ReturnTrack=int(((endCyl-starCyl)*TrkOffset)/200+starCyl)
  
  if echo=='on':
    print hd,"\t",zn,"\t\t",starCyl,"\t\t\t",endCyl
    print "TestTrack\t" ,ReturnTrack
...
def swdErrorRate (startCyl, endCyl, head):
  ...
  @return: Error code
  """
  mstartCyl, lstartCyl = lwords(startCyl)
  mendCyl, lendCyl = lwords(endCyl)
...
def setFlyHeight (cylinder, head, flyHeightPct = 0, options = "W"):
  ...
  @return: Sense Data
  """
  mstartCyl, lstartCyl = lwords(cylinder)
...
def setEchoServoCmdOn():
  ...
  SRVO: xxxx xxxx xxxx xxxx xxxx xxxx  xxxx xxxx xxxx xxxx xxxx
  @return: Error code
  """
  buf,errorCode = fn(9901,1)
  #buf,errorCode = __ReceiveResults()
...
def AddServoFlaw( cylinder, head, wedge ):
  """
  ...
  mCylinder, lCylinder = lwords(cylinder)
  buf, errorCode = fn( 1314, mCylinder, lCylinder, head, wedge )
  #buf,errorCode = __ReceiveResults()
  __displayBuffer(buf)
  return errorCode
####################################################################
...
def GetFitness(flags =0, channel=AGERE):
  ...
  @param flags: User input for command options (Bit 0 = FITNESS_NO_VGAR  -   Skip VGAR statistical calculations to save time.)
                                               (Bit 1 = FITNESS_NO_TAPS  -   Skip TAPS statistical calculations to save time.)
  @param channel: Set to 0 for Agere/LSI by default, change to 1 to display Marvell
  @return: Error code
  """
...
def saveAP (targetfile, rcsFile):
    ...
    filefound = 0
    path = 'c:\\var\\merlin\\results\\'
    fileName = r'c:\var\merlin\results\%s-%s\%s\%s'%(CellIndex, TrayIndex, 0, targetfile,)
    if os.path.exists(fileName): raise Exception("%s already exists" % fileName)
   
    if rcsFile == fileType.RAP or rcsFile == fileType.SAP or rcsFile == fileType.CAP:
       if not UPSEnabled: st(178,CWORD1= rcsFile | 0x0008, timeout=100)     # save the data from flash to the PC file
       else:              st(178,Cwrd1 = rcsFile | 0x0008, timeout=100)
    
    elif rcsFile == fileType.BIN:
       if not UPSEnabled: st(178,CWORD4=0x0001)                             # save the flash image to the PC file
       else:              st(178,Cwrd4=0x0001)
       
    PCfile = "%s-%s-%s"%(CellIndex, TrayIndex,0)
    if rcsFile == fileType.RAP:
      binf = open('c:\\var\\merlin\\pcfiles\\rapdata\\' + PCfile,'rb')
    elif rcsFile == fileType.SAP:
      binf = open('c:\\var\\merlin\\pcfiles\\sapdata\\' + PCfile,'rb')
...
def BCITrackRead (Cyl, Head, StartSector = 0, NumberOfSectors = -1, Options = 0, treadORreadcont=1):
  """
  ...
  if treadORreadcont==1:
    errorCode = tread(StartSector, NumberOfSectors)
  else:
    errorCode = readcont(StartSector, NumberOfSectors)
  if errorCode:
     print "Read Failure"
     return errorCode
  else:
...
def DumpBCILogData (FirstSLE = 0, NumberOfSLEs = 0xFFFF, SLESizeInBytes = 8):
  """
...
#     print "Current Entry Ptr\t%u" % log_status[4]
     ...
     MaxSLEsPerBlock = 512 / SLESizeInBytes 
     ...
     while (NumberOfSLEsToGo):
        ...
        # Request the data
        buf, errorCode = fn(1225, StartSLE, SLERecordCnt)
...
def getZoneFrequency(Head = 0xFF, Zone = 0xFF):
  ...
  dataRate = struct.unpack(buf[0:4])
...
def SetFlawscanSetting(defect_length=0x0305, Upper_Threshold=127, Lower_Threshold=0xB,TA_Threshold=0x0011,Enable=0,EnableTA=0):
  """
  Set the Data Flawscan Setting
  Upper byte maps to QM_TOL. Lower byte maps to QM_LEN. 
  A defect is detected if QM_TOL errors in the last QM_LEN peaks are < QM_LVL. 
  @return: Error code
  """
  buf,errorCode = fn (6666, defect_length, Upper_Threshold, Lower_Threshold, TA_Threshold, Enable, EnableTA)
  #buf,errorCode = __ReceiveResults()
  print "hello"
  print buf
  return errorCode    
...
### Function : readChnlSymb
def readChnlSymb_HC(SymbAddr,SymbMask,SymbLoc):
 
 buf,Erro=fn(1308, SymbAddr)
 RegValue  = int(binascii.hexlify(buf), 16)
 SymbValue = ((65535-SymbMask) & RegValue) >> SymbLoc
 
 return SymbValue
 
### Function : writeChnlSymb
def writeChnlSymb_HC(SymbAddr, SymbMask,SymbLoc,Value):
 
 
 buf,Erro=fn(1308, SymbAddr)
 RegValue  = int(binascii.hexlify(buf), 16)
 #print RegValue
 NewRegValue    = (SymbMask & RegValue) | (Value << SymbLoc)
 #print NewRegValue
 buf,err=fn(1269, SymbAddr,NewRegValue)
 HexNewRegValue=binascii.hexlify(buf)
 __displayMemory(buf,SymbAddr,addressableUnit=2,forceLen=2)

####################################################################
def ujog_MAT(start_offset, end_offset, step_size, mode = "BER",targetBer=90, numRevs=0, TLevel=0, ldpc=1,globalIterations=1, localIterations=0x55,zeroLatency=0):
  """
  Microjog: Measure BER or CQM versus offtrack percent on the ActiveHead, ActiveCylinder.
  Display offtrack, BER or CQM, Data Errors, Sync Errors, Other Errors, and Sense Code.
  @param start_offset: starting offtrack - OD side
  ...
  #ber=[0.0]*((end_offset-start_offset)/step_size+1) #Creating empty array to hold BER values for each offset
  ber=list()
  secPerRev=list()
  #data_errors=list()
  #sync_errors=list()
  ...
  while end_offset >= offset and errorCode == 0:
    ...
    if mode == "BER":
        if ((TLevel % 2) != 0) or ((TLevel > 30) and (TLevel != 100)):
            ...
            return
        if ldpc & 1:
            zeroLatency |= 4   # Use this flag to set the BER option for execfunc.c
        if ldpc & 2:
            zeroLatency |= 2   # Use this flag to set the BER option for execfunc.c
        ...
        if ldpc != 0:
            other_errors = result[4:5]
            print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " ECC TLEVEL = %3u" %TLevel + " BER = %f " % (result[0:1]) + " Bit Errors = %u " % (result[1:2])+" Data Errors = %u " % (result[2:3]) + " Sync Errors = %u " % (result[3:4])  + " Sectors per rev = %u " % (result[5:6])
            ScriptComment("Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " ECC TLEVEL = %3u" %TLevel + " BER = %f " % (result[0:1]) + " Bit Errors = %u " % (result[1:2])+" Data Errors = %u " % (result[2:3]) + " Sync Errors = %u " % (result[3:4])  + " Sectors per rev = %u " % (result[5:6]))
            #print "BER = %f " % (result[0:1]) + "Bits in Error = %u " % (result[1:2]) + "Data Errors = %u " % (result[2:3]) + "Sync Errors = %u " % (result[3:4]) + \"Other Errors = %u " % (result[4:5]) + "Sectors per Rev = %u " % (result[5:6]) + "Sense Data = %x " % (result[7:8])
        else:
            other_errors = result[4:5]
            print "Servo Offset: %3u" % offset + " Offtrack Pct: %7.3f" % offtrack + " ECC TLEVEL = %3u" %TLevel + " BER = %f " % (result[0:1]) + " Data Errors = %u " % (result[2:3]) + " Sync Errors = %u " % (result[3:4])  + " Sectors per rev = %u " % (result[5:6])
            #print "BER = %f " % (result[0:1]) + "Data Errors = %u " % (result[2:3]) + "Sync Errors = %u " % (result[3:4]) + \"Other Errors = %u " % (result[4:5]) + "Sectors per Rev = %u " % (result[5:6]) + "Sense Data = %x " % (result[7:8])
        if other_errors[0] > 0:
            if ldpc != 0:
                print "Failed Sector = %u" % (result[8:9])
            else:
                print "Failed Sector = %u" % (result[8:9])
    ber.append(result[0])
    secPerRev.append(result[5])
    #data_errors.append(result[1:2])
    #sync_errors.append(result[2:3])
    offset = offset + step_size
  
  ofs(0)
  return (ber,secPerRev)
  #return errorCode
####################################################################
